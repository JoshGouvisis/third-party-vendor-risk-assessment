...
Before this project, I thought of vendor risk assessment as mostly a checkbox exercise: does the vendor have SOC 2, do they encrypt data, do they have a backup plan. Working through CloudVault HR's questionnaire responses in detail showed me that the real analytical work happens in the gaps between what a vendor says and what that actually means in practice.

The MFA finding was the most straightforward, but it taught me something important about how to read vendor responses. CloudVault HR did not lie. They said MFA is available and recommended. But in a financial services context handling SSNs and bank account data, "available and not enforced" is a risk decision, not a security control. A vendor can have every tool in place and still leave a door open by making enforcement optional. That distinction between capability and enforcement is something I'll look for in every assessment going forward.

The SOC 2 coverage gap was more nuanced. On the surface, SOC 2 Type II sounds like strong assurance, and in many contexts it is. But the trust service criteria that were not covered, Confidentiality and Privacy, are exactly the ones most relevant to what CloudVault HR does. I learned that you cannot evaluate a compliance certification in isolation. You have to ask what it actually covers and whether that coverage matches the risk profile of the engagement.

The 2024 incident was the most interesting part to analyze. My first instinct was that any breach history is a red flag. But when I looked at what actually happened, a staging environment with test data, disclosed publicly, remediated in 72 hours, I came to the opposite conclusion. A vendor that discloses quickly and remediates thoroughly is demonstrating a security culture that I would rather work with than a vendor with a clean history that has never been tested. That reframe was genuinely new thinking for me.
